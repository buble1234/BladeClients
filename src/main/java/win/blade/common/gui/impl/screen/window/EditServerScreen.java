...
public class EditServerScreen extends BaseWindowScreen {
    ...
    public EditServerScreen(Screen parent, ServerInfo serverInfo, ServerList serverList) {
        super(Text.of("Editing Server"), 306, 122);
        this.parent = parent;
        this.serverI = serverInfo;
        this.serverL = serverList;
        this.originalAddress = serverInfo.address;
    }
    ...
    private void saveAndClose() {
        String newAddress = ip.getText();
        serverI.name = name.getText();
        serverI.address = newAddress;

        if (!newAddress.equals(this.originalAddress)) {
            LOGGER.info("Server address changed from '{}' to '{}'. Resetting favicon before saving.", this.originalAddress, newAddress);
            serverI.setFavicon(null);
        } else {
            LOGGER.info("Server address was not changed. Keeping existing favicon data.");
        }

        serverL.saveFile();
        LOGGER.info("Server list saved with corrected icon data.");
        close();
    }
