 ...
 */
public class HeadPointProvider implements PointProvider {
    ...
    public Vec3d getPoint(Entity entity) {
        ...
        double highestY = safeBox.maxY;
        Box headBox = safeBox.withMaxY(highestY).withMinY(highestY - 0.4);
        headBox = headBox.contract(SHRINK_FACTOR, 0.0, SHRINK_FACTOR);

        Vec3d targetVelocity = livingEntity.getPos().subtract(livingEntity.getPos());
        headBox = headBox.offset(targetVelocity.multiply(0.4));

        Vec3d bestPoint = findBestVisiblePoint(headBox, eyePos);
        return bestPoint != null ? bestPoint : headBox.getCenter();
    }
