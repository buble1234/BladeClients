 ...
 */
public class BodyPointProvider implements PointProvider {
    ...
    public Vec3d getPoint(Entity entity) {
        ...
        double centerY = safeBox.getCenter().y;
        Box bodyBox = safeBox.withMaxY(centerY + 0.5).withMinY(centerY - 0.5);
        bodyBox = bodyBox.contract(SHRINK_FACTOR, 0.0, SHRINK_FACTOR);

        Vec3d targetVelocity = livingEntity.getPos().subtract(livingEntity.getPos());
        bodyBox = bodyBox.offset(targetVelocity.multiply(0.4));

        Vec3d bestPoint = findBestVisiblePoint(bodyBox, eyePos);
        return bestPoint != null ? bestPoint : bodyBox.getCenter();
    }
